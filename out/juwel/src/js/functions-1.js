...
$(function(){
	...
	juwel.productCarousel = function() {
...
// ------ end --- Product-Detail
	...
	juwel.accessoriesSlider = function( opts ) {
	...
	// ----- Lightbox Gallery
	juwel.galleryBox = function(box, startWith) {
	...
	juwel.accessoriesDetail = function() {
		var main = $('#main');
		...
		juwel.accessoriesSlider({width: 538, items: 3}); // bottom accessories-slider
		...
		juwel.addedToCartBox = function() {
			...
			$.colorbox({transition:'elastic', speed:300, opacity: 0.5, overlayClose: false, scrolling: false, width: 640, height: 560, inline:true, href:"#added-to-cart",
								onComplete:function(){ $('#colorbox').addClass('standard');
													   ...
													   juwel.accessoriesSlider({width: 538, items: 3}); // product-suggestion slider
								...
								onClosed:function(){	juwel.accessoriesSlider({width: 538, items: 3}); // reinitialise slider in main-page
	...
	juwel.contentLightBox = function(content) {
	...
	juwel.myjuwel = function() {
		...
		$('.myjuwel-newsletter .jqTransformSelectWrapper ul li a').on('click', function(){
			newsletter() // check newsletter state
		});

		/* check newsletter state*/
		function newsletter(){
			var yes_sel = $('.myjuwel-newsletter .jqTransformSelectWrapper ul li a[index="1"]') // positive selector

			if(!(yes_sel.hasClass('selected'))){ // "no" is selected
				$('.myjuwel-newsletter .form-text').hide();
			}else{ // "yes" is selected
				$('.myjuwel-newsletter .form-text').show();
			}
		}
	...
	function panel_cart(){
		...
		.find(".item-delete, this")
				.tipsy({offset: 3, gravity: 'e', delayIn: 10, delayOut: 10, container: 'https://www.juwel-aquarium.de/out/juwel/src/js/.has-drop.cart .drop'}) // bind the tooltip
		...
		*/
		$('#panel-nav .panel-cart').on('click', 'https://www.juwel-aquarium.de/out/juwel/src/js/.button.grey',function(){ // close cart on click
	...
	$('.product-act-nav li.share')
		.mouseenter(
			function () {
				...
				$('#main .product-detail-stage').find('https://www.juwel-aquarium.de/out/juwel/src/js/.caroufredsel_wrapper, .stage-slider, .item').css('overflow', 'visible');

		}).mouseleave(
			function(){
				$(this).children('.bubble').hide();
				$('#main').css('overflow-y', 'visible');
				$('#main .product-detail-stage').find('https://www.juwel-aquarium.de/out/juwel/src/js/.caroufredsel_wrapper, .stage-slider, .item').css('overflow', 'hidden')
			});
